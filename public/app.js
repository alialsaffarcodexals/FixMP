...
function loadSound(name){
  ...
  for(const ext of exts){
   ...
   if(a.canPlayType(`audio/${ext}`)){
      try{ ctx.createMediaElementSource(a).connect(ctx.destination); }catch(e){}
      return a;
    }
  }
  return null;
...
document.addEventListener('click', (e)=>{
  ...
  if(!btn) return;
  if(btn.dataset.role==='roll') return;
  playSnd(clickSfx);
});
function playSnd(a){
...
function renderSidebar(){
  ...
  $('#btn-roll').disabled = state.hasRolledThisTurn;
...
function onRoll(){
    if(state.isRolling || state.hasRolledThisTurn) return;
